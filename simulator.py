...
class Simulator:
    ...
    def __init__(self, field: Field = None, cars: List[Car] = None):
        ...
        # (x, y) -> (car1, car2, 1 or 0, direction, step), where 1 means collision, 0 means complete
        self.end_state = {}
        self.end_print = ""
    ...
    def forward(self, car: Car) -> None:
        ...
        if command == "F":
            ...
            x += dx
            y += dy
            if not (x < 0 or x >= self.field.width or y < 0 or y >= self.field.height):
                car.x, car.y = x, y
            else:
        ...
        while len(self.cars):
            ...
            for i, car in enumerate(self.cars):
                ...
                if (car.x, car.y) in curr_positions:
                    colliding_car = self.cars[curr_positions[car.x, car.y]]
                    self.end_print += (
                        f"{colliding_car.name}, collides with {car.name} "
                        f"at ({car.x}, {car.y}) at step {step}\n"
                    )
                    self.end_print += (
                        f"{car.name}, collides with {colliding_car.name} "
                        f"at ({car.x}, {car.y}) at step {step}\n"
                    )
